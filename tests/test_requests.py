...
class TestInsanicRequest:
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                {settings.FORWARDED_FOR_HEADER: ip_generator(2, True)},
                "2.2.2.2",
            ),
            ({settings.FORWARDED_FOR_HEADER: ip_generator(3)}, "1.1.1.1"),
            (
                {settings.FORWARDED_FOR_HEADER: ip_generator(3, True)},
                "3.3.3.3",
            ),
        ),
    )
    ...
    def test_remote_addr_with_proxies(
        self, set_proxies_count, headers, expected
    ):
        headers = CIMultiDict(headers)
