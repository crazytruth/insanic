...
class TestMockService:
    ...
    @pytest.mark.parametrize(
        "endpoint", ["/api/v1/{}", "/api/v1/{}?endpoint=this"]
    )
    @pytest.mark.parametrize(
        "request_body", [{}, {"request": "body", "body": "request"}]
    )
    @pytest.mark.parametrize(
        "query_params", [{}, {"query": "params", "params": "query"}]
    )
    async def test_register(
        self, method, endpoint, request_body, query_params, function_session_id
    ):
        ...
        self.mock_service.register_mock_dispatch(
            method,
            endpoint,
            expected_response,
        ...
        response = await self.mock_service.mock_dispatch(
            method, endpoint, query_params=query_params, payload=request_body
        )
        assert response == expected_response
        response = await self.mock_service.mock_dispatch(
            method, endpoint, query_params=query_params
        )
        assert response == expected_response
        response = await self.mock_service.mock_dispatch(
            method, endpoint, payload=request_body
        )
        assert response == expected_response
