...
def _extract_changelog(data):
    ...
    for h in data["headings"]:
        if (
            h["version"] in data["original_version"]
            or h["version"] == data["new_version"]
        ):
            start_line = h["line"]
        else:
            try:
...
def _prepare_slack(new_version, data):
    ...
    params["text"] = (
        f"Gotta go insanely fast! New version [{new_version}] has been released. "
        f"`pip install -U insanic` to update."
    )
    params["icon_emoji"] = ":sanic:"
    params["attachments"] = []
    params["attachments"].append(
        {"text": _beautify_changelog_for_slack(changelog), "mrkdwn": True}
    )
